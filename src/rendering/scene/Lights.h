...
/// A representation of a DirectionalLight render scene element for task h
struct DirectionalLight {
    ...
    static DirectionalLight off() {
        return {glm::vec3{0.0f, -1.0f, 0.0f}, glm::vec4{0.0f}};  // default "down" and no color
    }
...
struct LightScene {
    ...
    // added declaration so the scene knows about directional lights for task h
    ...
    // added new declaration for task h
    std::vector<DirectionalLight> get_all_directional_lights(size_t max_count, size_t min_count = 0) const;
